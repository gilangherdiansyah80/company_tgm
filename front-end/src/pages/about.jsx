...
const About = () => {
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`http://localhost:3000/api/v1/about/delete/${id}`, {
                method: 'DELETE',
            });
                        ...
                        {dataAbout.map((item, index) => (
                            <div key={item.id} className='bg-gradient-to-l from-[#67BD5E] to-[#467840] rounded-xl p-3 flex flex-col text-white gap-y-3'>
                                ...
                                <div className='flex w-full gap-x-3'>
                                    <Link to={`/editabout/${item.id}`} className='w-1/2 bg-white p-3 text-black rounded-lg text-center'>
